 ...
 * @Description: 用于生成Mapper XML文件
 */

public class BuildMapperXml {

    // 五个字符串常量，用于在生成Mapper XML文件时引用不同的元素和SQL片段
    ...
    public static void execute(TableInfo tableInfo) {
        ...
        try {
            ...
            // 写入了Mapper XML文件的头部信息，包括XML版本和DTD定义
            ...
            for (FieldInfo FieldInfo : fieldInfoList) {
                ...
                if (FieldInfo.getAutoIncrement() != null && FieldInfo.getAutoIncrement()) {
                    bw.write("\t\t<id column=\"" + FieldInfo.getFieldName() + "\" property=\""
            ...
            for (FieldInfo FieldInfo : fieldInfoList) {
                ...
                if (Constants.TYPE_STRING.equals(FieldInfo.getJavaType())
                        || Constants.TYPE_DATE.equals(FieldInfo.getJavaType())) {
            ...
            for (FieldInfo FieldInfo : fieldInfoList) {
                if (Constants.TYPE_STRING.equals(FieldInfo.getJavaType())) {
                    bw.write("\t\t\t<if test=\"query." + FieldInfo.getPropertyName() + Constants.SUFFIX_PROPERTY_FUZZY
                            + "!= null  and " +
                            "query." + FieldInfo.getPropertyName() + Constants.SUFFIX_PROPERTY_FUZZY + "!=''\">");
                    bw.newLine();
                    bw.write("\t\t\t\t and  " + FieldInfo.getFieldName() + " like concat('%', #{query."
                            + FieldInfo.getPropertyName() + Constants.SUFFIX_PROPERTY_FUZZY + "}, '%')");
                    bw.newLine();
                    bw.write("\t\t\t</if>");
                    bw.newLine();
                }
                ...
                if (Constants.TYPE_DATE.equals(FieldInfo.getJavaType())) {
                    ...
                    bw.newLine();
                    bw.write("\t\t\t\t <![CDATA[ and  " + FieldInfo.getFieldName() + ">=str_to_date(#{query." + start
                            + "}, '%Y-%m-%d') ]]>");
                    bw.newLine();
                    bw.write("\t\t\t</if>");
                    bw.newLine();
            ...
            if (autoIncrementColumn != null) {
                bw.write("\t\t<selectKey keyProperty=\"bean." + autoIncrementColumn.getPropertyName()
                        + "\" resultType=\"" + autoIncrementColumn
                                .getJavaType()
                        + "\" order=\"AFTER\">");
                bw.newLine();
            ...
            bw.newLine();
            bw.write("\t<insert id=\"insertOrUpdate\" parameterType=\"" + Constants.PACKAGE_BEAN + "."
                    + tableInfo.getBeanName() + "\">");
            bw.newLine();
            bw.write("\t\t INSERT INTO " + tableInfo.getTableName());
            bw.newLine();
            ...
            bw.newLine();
            bw.write("\t<insert id=\"insertOrUpdateBatch\" parameterType=\"" + Constants.PACKAGE_BEAN + "."
                    + tableInfo.getBeanName() + "\">");
            bw.newLine();
            bw.write("\t\t INSERT INTO " + tableInfo.getTableName());
            bw.write("(");
            ...
            bw.write("\t<!--多条件修改-->");
            bw.newLine();
            bw.write("\t<update id=\"updateByParam\" parameterType=\"" + Constants.PACKAGE_PARAM + "."
            ...
            for (Map.Entry<String, List<FieldInfo>> entry : keyMap.entrySet()) {
                ...
                if (whereStr.length() > 0) {
                    ...
                    bw.newLine();
                    bw.write("\t<update id=\"updateBy" + methodName + "\" parameterType=\"" + Constants.PACKAGE_BEAN
                            + "." + tableInfo
                                    .getBeanName()
                            + "\">");
                    bw.newLine();
                    ...
                    bw.newLine();
                    bw.write("\t\tselect <include refid=\"" + BASE_COLUMN_LIST + "\" /> from "
